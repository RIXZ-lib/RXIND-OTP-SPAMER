#!/usr/bin/env python3
# handlers.py - RXIND OTP HANDLERS
# Total: 60 API Handlers
...
# 26. Termii OTP
def send_termii_otp(phone_number):
    ...
    payload = {
        "phone_number": phone_number,
        "api_key": "tlv_un2cAjeNmdp6UHFWFKSlIN4GmF73lBTR7e2yB5oLFO0"
    }
    try:
        resp = requests.post(url, headers=headers, json=payload, timeout=15)
        return resp
...
# 27. VirtualSMS OTP
def send_virtualsms_otp(phone_number):
...
# 29. Matahari OTP
def send_matahari_otp(phone_number):
...
# 30. Rumah123 OTP
def send_rumah123_otp(phone_number):
...
# 31. Paper.ID OTP
def send_paperid_otp(phone_number):
...
# 32. Sahabat Daihatsu OTP
def send_sahabatdaihatsu_otp(phone_number):
...
# 33. Sayurbox WA
def send_sayurbox_otp(phone_number):
...
# 34. Carsome WA
def send_carsome_otp(phone_number):
...
# 35. Jenius WA
def send_jenius_otp(phone_number):
...
# 36. Gojek WA
def send_gojek_otp(phone_number):
...
# 37. Klikwa WA
def send_klikwa_otp(phone_number):
...
# 38. Alodokter OTP
def send_alodokter_otp(phone_number):
...
# 39. Klikdokter OTP
def send_klikdokter_otp(phone_number):
    ...
    data = {
        "_token": ''.join(random.choices(string.ascii_letters + string.digits, k=40)),
        "full_name": "BambangSubianto",
        "email": f"user{random.randint(100,999)}@mailnesia.com",
        "phone": phone_number,
        "back-to": "",
        "submit": "Daftar"
    }
...
# 40. Prosehat OTP
def send_prosehat_otp(phone_number):
...
# 41. Hijup OTP
def send_hijup_otp(phone_number):
    ...
    payload = f'[{{"phone_number":"{phone_number}","store_path":"hijup"}}]'
    try:
...
# 42. Alodokter OTP V2
def send_alodokter_v2_otp(phone_number):
...
# 43. Blibli Tiket OTP
def send_bliblitiket_otp(phone_number):
...
# 44. Ohsome OTP
def send_ohsome_otp(phone_number):
...
# 45. Optik Melawai OTP
def send_optikmelawai_otp(phone_number):
...
# 46. Holland Bakery OTP
def send_hollandbakery_otp(phone_number):
